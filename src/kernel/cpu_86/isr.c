...
void isr_install()
{
    ...
    set_idt_gate(128, (uint32_t)syscall128);
...
void syscall_handler(interrupt_info_t* info)
{
    ...
    if (info->eax > SYSCALL_NUM || !syscall_handler)
    {
        ...
        printf("EAX=0x%x SYSCALL IS OUT OF RANGE OR ISN'T DEFINED!", info->eax);
...
{
    if (info->int_no >= 40) outb(0xA0, 0x20);
    outb(0x20, 0x20);

    if (interrupt_handlers[info->int_no] != 0) 
    {
        isr_t handler = interrupt_handlers[info->int_no];
        handler(info);
    }
}
